...
`src/lib/contactValidation.ts` is the single source of truth for the public contact form's rules — imported by both `ContactForm.tsx` ('use client', for `pattern`/`minLength`/`maxLength` attributes + translation-key selection) and `contact.ts`'s `submitContactForm` Server Action (authoritative check; the client attributes are UX only, since `submitContactForm` is a public, unauthenticated endpoint reachable directly). Each field validator returns a `ContactFieldErrorCode` (`'required' | 'tooShort' | 'tooLong' | 'invalidFormat'`) rather than a boolean, so `ContactForm.tsx` can show the exact right message per field via `t(\`nameError_${code}\`)` (translated client-side, not server-side — avoids depending on next-intl's server-side locale context inside a `useActionState`-invoked action).

- **Limits:** name 2–40 chars, message 10–500 chars (deliberately tight — this form is filled and read mostly on mobile). Phone: 7–15 digits after stripping formatting (E.164's real-world bound), plus a 7–25 total-character ceiling (`PHONE_MIN_LENGTH`/`PHONE_MAX_LENGTH`) that's independent of the digit-count check — this catches a technically-15-digit-but-absurdly-padded string (e.g. digit-by-digit spacing) that the digit-count check alone wouldn't reject.
- **Phone format is deliberately international/permissive, not Bulgaria-only-strict** — accepts `0876346261`, `+359876346261`, `00359876346261`, and any other country's number (character class `[+0-9\s()-]` + digit-count range). A dental clinic doesn't want to turn away a real inquiry over number format.
- **Sanitization (`sanitizeName`/`sanitizePhone`/`sanitizeMessage`) strips ASCII control characters before validation.** This isn't a general XSS defense — React already escapes all rendered submission fields (confirmed no `dangerouslySetInnerHTML` in that render path) and `JSON.stringify` already escapes for the `data/contact-submissions.json` write. The actual motivation: `ntfy.ts`'s `buildMessage()` naively joins raw field values into a plain-text, newline-delimited notification body — a name/message containing a raw `\n` could otherwise inject a fake extra line (e.g. spoofing a `Телефон:` line) into the doctor's push notification. Name/phone strip all control chars (single-line fields); message keeps real `\n`/`\t`/`\r` since it's genuinely multi-line.
- **`submitContactForm` reads `FormData` via a local `getStringField()` helper, not a raw `formData.get(x) as string` cast** — `FormData.get()`'s real type is `File | string | null`, and since this action is public/unauthenticated, a crafted multipart POST could send a `File` under any field name; an unguarded cast throws at runtime instead of returning a clean validation error.
- **e2e tests that deliberately submit invalid data are safe from spamming ntfy for two independent reasons:** `submitContactForm` returns before `appendContactSubmission`/`sendNtfyNotification` the moment any field fails validation (so an always-invalid test submission never reaches ntfy at all), *and* `sendNtfyNotification` itself no-ops during any e2e run (checks for a `.e2e-running` marker file). The 3 field-validation tests in `e2e/contact.spec.ts` rely on the first guarantee by construction — each submits exactly one deliberately-invalid field, never a fully valid combination.
