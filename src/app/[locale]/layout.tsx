...
import dynamic from 'next/dynamic'; // Added for optimization
import '../globals.css';
...
// STEP 1: Dynamically import heavy/non-critical client components
// This keeps them out of the initial mobile "Main Thread" work.
const BackToTop = dynamic(() => import('@/components/BackToTop'), { ssr: false });
const FloatingCTA = dynamic(() => import('@/components/FloatingCTA'), { ssr: false });
const CookieConsent = dynamic(() => import('@/components/CookieConsent'), { ssr: false });
const StatisticsLink = dynamic(() => import('@/components/StatisticsLink'), { ssr: false });

const playfair = Playfair_Display({
    subsets: ['latin'],
    display: 'optional', // 'optional' prevents layout shift (CLS)
    preload: true,
});
    ...
    return (
        <html lang={locale}>
            <body className={playfair.className}>
                        ...
                        <div className='flex items-start flex-shrink-0 lg:w-64 absolute left-1/2 -translate-x-1/2 lg:relative lg:left-0 lg:translate-x-0 pt-1 lg:pt-0'>
                            ...
                            >
                                <Image
                                    ...
                                    width={400} // Reduced from 843 to save memory/processing
                                    height={190}
                                    priority
                    ...
                    <div className='max-w-7xl mx-auto pt-6 md:pt-8 pb-2 md:pb-3 px-4 sm:px-6 lg:px-8'>
                        <div className='flex flex-col items-center space-y-4'>
                            <div className='flex space-x-4 md:space-x-6'>
                                <a href='https://facebook.com/...' target='_blank' rel='noopener' className='text-gray-600 hover:text-[#1877F2]'>
                                    <svg className='w-6 h-6' fill='currentColor' viewBox='0 0 24 24'>
                                        <path d='M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z' />
                                    </svg>
                                </a>
                                <a href='https://instagram.com/...' target='_blank' rel='noopener' className='text-gray-600 hover:text-[#E4405F]'>
                                    <svg className='w-6 h-6' fill='currentColor' viewBox='0 0 24 24'>
                ...
                {/* These are now dynamic, non-blocking */}
                <BackToTop />
